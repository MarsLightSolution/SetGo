...
const handleChange = async (e) => {
  ...
  if (name === "pictures" && files) {
    ...
    for (let file of filesArray) {
      ...
      if (file.size / 1024 / 1024 > 2) {
        showErrorToast(`${file.name} ${t("form.maxFileSizeError") || "exceeds 2 MB"}`);
  ...
  color="success"
  className="mt-8"
  disabled={loading} // <-- disables button when loader is active
  sx={{
    opacity: loading ? 0.6 : 1, // dimmed effect
