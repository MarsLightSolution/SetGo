      ...
      <div className="flex flex-wrap gap-4 justify-center md:justify-start">
        {ads.map((ad, index) => (
          <AdCard
            ...
            image={
              ad.image ||
              `${process.env.NEXT_PUBLIC_VITE_SERVER}/${ad.pictures?.[0]?.replace(/\\/g, "/") || "uploads/placeholder.jpg"}`
            }
            price={ad.price}
          />
        ))}
...
const Home = () => {
  ...
  const fetchProducts = async (type, page) => {
    try {
      ...
      // Endpoint
      let endpoint = `${process.env.NEXT_PUBLIC_VITE_SERVER}/api/products/getProducts`
      if (type === "nearby" && location?.latitude && location?.longitude && radius > 0) {
        endpoint = `${process.env.NEXT_PUBLIC_VITE_SERVER}/api/products/nearby`
      }
  ...
  const fetchGalleryData = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_VITE_SERVER}/api/products/getProducts?limit=10`, {
        method: "GET",
        credentials: "include",
      })
      ...
      const mappedProducts = products.map((product) => ({
        ...
        image: product.pictures?.[0]
          ? `${process.env.NEXT_PUBLIC_VITE_SERVER}/${product.pictures[0].replace(/\\/g, "/")}`
