...
const Home = () => {
  ...
  const fetchGalleryData = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SERVER}/api/products/`, {
        method: "GET",
        credentials: "include",
      })
...
<div className="bg-white p-4 rounded-2xl shadow-lg border border-gray-200 flex-1 w-full md:w-[60%]">
  <div className="flex justify-between items-center mb-4">
    <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
      <ImageIcon className="w-5 h-5 text-green-600" /> {t("home.gallery")}
    </h2>
    ...
    {galleryLoading ? (
      ...
      galleryData.map((item, index) => (
        <div
          ...
          className="min-w-[180px] max-w-[200px] flex-shrink-0 bg-white border-2 border-gray-100 rounded-xl shadow-sm hover:shadow-lg cursor-pointer transition-all duration-300 hover:scale-105 group relative overflow-hidden"
        >
          {/* Icon frame */}
          <div className="absolute top-2 left-2 bg-white shadow-md rounded-full p-1">
          ...
          {/* Hover overlay */}
          <div className="absolute bg-black bg-opacity-0 group-hover:bg-opacity-5 transition-all duration-300 rounded-xl"></div>
        </div>
      ))
    ) : (
      <div className="flex items-center justify-center w-full h-[200px]">
