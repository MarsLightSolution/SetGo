...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 w-full bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b border-slate-200 ${scrolled ? "shadow-sm" : "shadow-none"} transition-shadow duration-300`}
      >
        {/* Top Bar */}
      ...
      <div
        className={`fixed inset-0 z-40 bg-black/30 md:hidden transition-opacity duration-200 ${mobileOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        aria-hidden="true"
        onClick={() => {
          setMobileOpen(false)
          setDropdownOpen(false)
      ...
      <div
        ...
        className={`md:hidden fixed inset-x-0 top-0 z-50 origin-top bg-white ring-1 ring-black/5 shadow-sm 
      ...
      >
        {/* Note: this wraps the original mobile content for smooth show/hide */}
        <div className="border-b border-slate-200">
          {/* reuse the existing mobile content from the header, without conditional rendering */}
