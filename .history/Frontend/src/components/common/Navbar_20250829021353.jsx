...
export default function Navbar() {
  ...
  useEffect(() => {
    if (mobileOpen) {
      ...
      // focus the mobile search shortly after panel opens
      const id = setTimeout(() => mobileSearchRef.current?.focus(), 150)
      return () => {
        clearTimeout(id)
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 w-full border-b border-slate-200 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/60 transition-[box-shadow,background-color] duration-300 ${
      ...
      >
        ...
        <div className="mx-auto max-w-screen-xl px-4 sm:px-6">
          <div className="flex h-16 items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <button
                className="inline-flex items-center justify-center rounded-md p-2 text-slate-700 hover:bg-slate-100 hover:text-emerald-600 md:hidden transition-colors"
          ...
          <div className="mx-auto max-w-screen-xl px-4 sm:px-6">
            <div className="flex h-12 items-center justify-between text-sm text-slate-700">
              <button
                ...
                className="inline-flex items-center gap-1 text-emerald-700 hover:text-emerald-600 transition-colors"
              >
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div
                    role="menu"
                    className="absolute right-0 mt-2 w-48 rounded-lg border border-slate-200 bg-white py-1 shadow-lg origin-top-right motion-safe:transition motion-safe:duration-200 motion-safe:ease-out motion-safe:opacity-100 motion-safe:scale-100"
                  >
        ...
        {/* Mobile overlay: fades in/out and closes the menu on click */}
        <div
          aria-hidden
          onClick={() => setMobileOpen(false)}
          className={`fixed inset-x-0 top-16 bottom-0 bg-black/30 md:hidden transition-opacity duration-300 ${
        ...
        <div
          ...
          className={`md:hidden border-t border-slate-200 overflow-hidden transition-[max-height,opacity,transform] duration-300 ease-out ${
            mobileOpen
        ...
        >
          <div className="mx-auto max-w-screen-xl px-4 sm:px-6 py-3 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <button
                onClick={() => navigate("/form")}
                className="inline-flex items-center gap-2 text-emerald-700 hover:text-emerald-600 transition-colors"
              >
                <MdOutlineAddCircle className="text-lg" />
                {t("navbar.advertise")}
              </button>
