...
    const [mapCenter, setMapCenter] = useState(location.latitude ? [location.latitude, location.longitude] : null);
...
                    dispatch(setLocation({ latitude: newLocation[0], longitude: newLocation[1] }));
                })
                .catch(error => {
...
                });
        }
    }, [isOpen, priceRange, condition, radius, city, searchQuery, location, t, dispatch]);
...
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-700 text-xl cursor-pointer" style={{ zIndex: 10 }}>
                        ×
                    </button>
                </div>
...
                    
                    {mapCenter && (
                        <div className="h-64 rounded-md overflow-hidden shadow-sm border border-gray-300">
                            <MapContainer
                                center={mapCenter}
...
                                {latestAds.map(product => {
                                    console.log("Rendering marker for product:", product.title, "at", product.location?.coordinates); // ✅ NEW CONSOLE LOG
...
                        className="w-full mt-2 accent-lime-500 cursor-pointer" 
                    />
                </div>
