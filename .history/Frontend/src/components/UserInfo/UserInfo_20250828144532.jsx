...
import { FaEdit, FaEye, FaPause, FaRocket, FaTrash } from "react-icons/fa" // Removed FaTimes import
...
export default function UserInfo() {
  ...
  const pauseAd = async (id) => {
    try {
      ...
      setAds((prev) => prev.map((ad) => (ad._id === id ? { ...ad, sold: !ad.sold } : ad)))
      toast.success(ads.find((ad) => ad._id === id)?.sold ? "Ad is now active!" : "Ad marked as sold!")
    } catch (err) {
                    ...
                    <div className="flex flex-col justify-between flex-grow p-4">
                      ...
                      <div className="flex gap-2 mt-4 justify-center">
                        ...
                        <button
                          ...
                          className={`${ad.sold ? "text-green-600 hover:bg-green-600" : "text-yellow-600 hover:bg-yellow-600"} bg-white border hover:text-white px-3 py-1.5 text-sm rounded-md shadow-sm transition`}
                          title={ad.sold ? "Activate Ad" : "Mark as Sold"}
                        >
                          {ad.sold ? <FaEye className="w-4 h-4" /> : <FaPause className="w-4 h-4" />}
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
