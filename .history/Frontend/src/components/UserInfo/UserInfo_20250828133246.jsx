...
import { FaEdit, FaEye, FaPause, FaRocket } from "react-icons/fa" // Replaced FaTrash with FaPause and added FaRocket for boost
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10">
      <div className="bg-white rounded-xl p-6 shadow-2xl w-[350px] text-center border border-gray-200">
        <h2 className="text-lg font-bold mb-3">{isBoost ? "Confirm Boost Ad" : "Confirm Pause Ad"}</h2>
        ...
        <div className="flex justify-center gap-4">
          ...
          <button
            ...
            className={`px-4 py-1.5 text-white rounded text-sm ${
              isBoost ? "bg-blue-600 hover:bg-blue-700" : "bg-yellow-600 hover:bg-yellow-700"
            }`}
          >
            {isBoost ? "Yes, Boost" : "Yes, Pause"}
          </button>
        </div>
      </div>
    </div>
...
export default function UserInfo() {
  ...
  const [confirmAction, setConfirmAction] = useState("pause") // Added state to track which action is being confirmed
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800">
      ...
      {confirmAdId && (
        <ConfirmDialog
          onConfirm={() => (confirmAction === "boost" ? boostAd(confirmAdId) : pauseAd(confirmAdId))}
