...
import { FaEdit, FaEye, FaPause, FaRocket, FaTrash, FaTimes } from "react-icons/fa" // Added FaTimes for close button
...
const ImagePreview = ({ imageUrl, onClose, title }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={onClose}>
      <div className="relative max-w-4xl max-h-[90vh] p-4">
        <button
          onClick={onClose}
          className="absolute top-2 right-2 z-10 bg-white/20 hover:bg-white/30 text-white rounded-full p-2 transition-colors"
        >
...
const ConfirmDialog = ({ onConfirm, onCancel, action = "pause" }) => {
  const { t } = useTranslation()
  const isBoost = action === "boost"
  const isPause = action === "pause"
  const isDelete = action === "delete" // Added delete action support

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10">
      <div className="bg-white rounded-xl p-6 shadow-2xl w-[350px] text-center border border-gray-200">
        <h2 className="text-lg font-bold mb-3">
          {isBoost ? "Confirm Boost Ad" : isPause ? "Confirm Pause Ad" : "Confirm Delete Ad"}
        </h2>
        <p className="text-sm text-gray-600 mb-5">
          {isBoost
            ? "Are you sure you want to boost this ad? This will increase its visibility."
            : isPause
              ? "Are you sure you want to pause this ad? It will be temporarily hidden from listings."
        ...
        <div className="flex justify-center gap-4">
          ...
          >
            {isBoost ? "Yes, Boost" : isPause ? "Yes, Pause" : "Yes, Delete"}
          </button>
        </div>
      </div>
    </div>
...
export default function UserInfo() {
  ...
  const pauseAd = async (id) => {
    try {
      await axios.patch(
        `${import.meta.env.VITE_SERVER}/api/products/product/${id}/pause`,
        {},
        {
      ...
      setAds((prev) => prev.map((ad) => (ad._id === id ? { ...ad, status: "paused" } : ad)))
      toast.success("Ad paused successfully!")
    } catch (err) {
      console.error("ERROR: Failed to pause product:", err.response ? err.response.data : err.message)
      toast.error("Failed to pause ad. Please try again.")
      if (err.response && err.response.status === 401) {
        navigate("/login")
      }
    } finally {
  ...
  const boostAd = async (id) => {
    try {
      await axios.patch(
        `${import.meta.env.VITE_SERVER}/api/products/product/${id}/boost`,
        {},
        {
          headers: { Authorization: `Bearer ${accessToken}` },
          withCredentials: true,
      ...
      setAds((prev) =>
        prev.map((ad) =>
          ad._id === id ? { ...ad, boosted: true, boostExpiry: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) } : ad,
        ),
      )
      toast.success("Ad boosted successfully! It will have increased visibility for 7 days.")
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800">
      ...
      {previewImage && (
        <ImagePreview imageUrl={previewImage.url} title={previewImage.title} onClose={() => setPreviewImage(null)} />
      )}
                    ...
                    <div className="h-44 w-full bg-gray-100 flex items-center justify-center overflow-hidden">
                      {ad.pictures?.[0] ? (
                        <img
                          ...
                          alt={displayTitle}
                          className="h-full w-full object-cover rounded-t-lg cursor-pointer hover:opacity-90 transition-opacity"
                          onClick={() =>
                            handleImagePreview(
                    ...
                    <div className="flex flex-col justify-between flex-grow p-4">
                      ...
                      <div className="flex gap-2 mt-4 justify-center">
                        ...
                        <button
                          ...
                          className="text-yellow-600 bg-white border hover:bg-yellow-600 hover:text-white px-3 py-1.5 text-sm rounded-md shadow-sm transition"
                          title="Pause Ad"
                        >
                        ...
                        <button
                          ...
                          className="text-blue-600 bg-white border hover:bg-blue-600 hover:text-white px-3 py-1.5 text-sm rounded-md shadow-sm transition"
                          title="Boost Ad"
                        >
