    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10">
      <div className="bg-white rounded-xl p-6 shadow-2xl w-[350px] text-center border border-gray-200">
        ...
        <div className="flex justify-center gap-4">
          ...
          >
            {isBoost ? "Yes, Boost" : isPause ? (adIsSellStatus ? "Yes, Pause" : "Yes, Resume") : "Yes, Delete"}
          </button>
        </div>
      </div>
    </div>
...
export default function UserInfo() {
  ...
  const pauseAd = async (id) => {
    try {
      ...
      const wasListed = currentAd?.isSell === true
      ...
      toast.success(wasListed ? "Listing paused!" : "Listing resumed!")
    } catch (err) {
                    ...
                    <div className="flex flex-col justify-between flex-grow p-4">
                      ...
                      <div className="flex gap-2 mt-4 justify-center">
                        ...
                        <button
                          ...
                          className={`${ad.isSell ? "text-yellow-600 hover:bg-yellow-600" : "text-green-600 hover:bg-green-600"} bg-white border hover:text-white px-3 py-1.5 text-sm rounded-md shadow-sm transition`}
                          title={ad.isSell ? "Pause Listing" : "Resume Listing"}
                        >
                          {ad.isSell ? <FaPause className="w-4 h-4" /> : <FaPlay className="w-4 h-4" />}
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
