...
// ✅ Connect socket only once, not on every conversation change
useEffect(() => {
  ...
  socket.on("newMessage", (msg) => {
    if (msg.conversationId === activeConversation?._id) {
      setMessages((prev) => {
        // avoid duplicate push if same message already exists
...
}, [isConnected, currentUser, activeConversation?._id]);
