...
export default function ChatApp() {
  ...
  // Socket setup (create once when connected + user available)
  useEffect(() => {
    ...
    // cleanup listeners on effect cleanup (component unmount or reconnect)
    return () => {
      socket.off("newMessage", handleNewMessage)
      socket.off("typing", handleTyping)
    }
  }, [isConnected, currentUser, SOCKET_URL])

  // Ensure we join/leave rooms when activeConversation changes
  useEffect(() => {
    if (!socketRef.current) return

    const convId = activeConversation?._id
    if (convId) socketRef.current.emit("joinConversation", convId)
    // Optionally, implement leave logic if you track previous room
  ...
  const connectUser = async (username) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Store userId & username in localStorage for auto-reconnect
  ...
  // Auto-connect on page load (fixed key name)
  useEffect(() => {
  ...
  const startConversation = async (otherUser) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (socketRef.current) socketRef.current.emit("joinConversation", data.conversation._id)
      }
    } catch (error) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const data = await response.json()
      if (data.success) {
        // optimistic UI: add immediately, but avoid duplicates via dedupe in socket handler
        const newMsg = {
          id: data.message._id,
        ...
        // Emit for realtime (server may also broadcast saved message)
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      // HEIC to JPEG conversion (frontend)
      ...
      if (data.success) {
        // Do NOT directly setMessages here to avoid duplicates — server will broadcast via socket
        if (socketRef.current) {
          socketRef.current.emit("sendMessage", {
            conversationId: activeConversation._id,
            senderId: currentUser.userId,
  ...
  // Helper to resolve file URLs (so you don't hardcode localhost)
