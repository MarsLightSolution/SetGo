// Make sure to install: npm install react-native-toast-message
// Update the import path below to match your Toastify component location
...
// Update this path to match your toast file location
import { 
  showSuccessToast, 
  showErrorToast, 
  showWarningToast, 
  ToastifyContainer 
} from '../components/Toastify'; // Adjust path as needed
import io from 'socket.io-client';
import * as ImagePicker from 'expo-image-picker';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { Ionicons } from '@expo/vector-icons';
...
export default function ChatApp() {
  ...
  const connectUser = async (username) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showErrorToast(data.message || 'Connection failed');
      }
    } catch (error) {
      console.error('Connection error:', error);
      setConnectionError('Connection failed. Check your network.');
      showErrorToast('Connection failed. Check your network.');
    } finally {
  ...
  const handleImageUpload = async () => {
    ...
    if (isUploading) {
      showWarningToast('Please wait for current upload to complete');
      return;
    }
    ...
    try {
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        quality: 0.7,
        base64: false,
        allowsMultipleSelection: false, // Only 1 photo allowed
      });

      if (result.canceled || !result.assets[0]) return;

      // Check if user selected more than 1 image
      if (result.assets.length > 1) {
        showErrorToast('You can only send 1 photo at a time');
        return;
      }
      ...
      // If fileSize not available, try to get it from file info
      ...
      if (fileSize && fileSize > MAX_SIZE_BYTES) {
        ...
        showErrorToast(`Image too large (${sizeMB}MB). Max ${MAX_SIZE_MB}MB allowed`);
        return;
      }
      ...
      try {
        ...
        // Remove temp message on parse error
      ...
      showErrorToast(error.message || 'Failed to send image. Please try again.');
    } finally {
