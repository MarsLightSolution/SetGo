...
const getPriorityProducts = asyncHandler(async (req, res) => {
  ...
  const matchStage = {
    ...
    priority: 1, // 🔥 Only products with priority true
  };

  pipeline.push({ $match: matchStage });

  // ✅ Sort: latest first (priority filter already applied)
  pipeline.push({
    $sort: { createdAt: -1 },
  });
  ...
  logger.info("[GetPriorityProducts] Priority products fetched", {
    total: result.totalProducts,
    currentPage: result.currentPage,
    totalPages: result.totalPages,
  });
  ...
  res
    .status(200)
    .json(new ApiResponse(200, result, "Fetched only priority products."));
});
