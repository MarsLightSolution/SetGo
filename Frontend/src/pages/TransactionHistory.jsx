import React, { useEffect, useState } from "react";

/**
 * TransactionHistory page – fetches the signed‑in user from localStorage
 * instead of expecting a :userId param in the route.  If you still want to
 * support /users/:id/transactions just pass a prop `forcedUserId`.
 */

const TransactionHistory = ({ forcedUserId }) => {
  /** 1️⃣  Grab user from localStorage (fallback to prop) */
  const [user, setUser] = useState(null);
  const [txns, setTxns] = useState([]);
  const [loading, setLoading] = useState(true);
  const [walletBalance, setWalletBalance] = useState(0);
  const [totalCredit, setTotalCredit] = useState(0);
  const [totalDebit, setTotalDebit] = useState(0);

  useEffect(() => {
    /* pull userData once */
    const stored = localStorage.getItem("userId");
    if (stored) {
      try {
        setUser(stored);
      } catch (e) {
        console.error("Error parsing userData", e);
      }
    }
  }, []);

  /** 2️⃣  Fetch transactions when user is available */
  useEffect(() => {
    const userId = user;
    if (!userId) {
      console.log(userId);
      return;
    } else console.log(userId);

    (async () => {
      try {
        const res = await fetch(
          `http://localhost:8080/users/${userId}/transactions`,
          {
            credentials: "include", // ✅ Important: to send cookies
          }
        );
        const json = await res.json();
        const { transactions, walletBalance, totalCredit, totalDebit } = json.data || {};

        setTxns(transactions || []);
        setWalletBalance(walletBalance || 0);
        setTotalCredit(totalCredit || 0);
        setTotalDebit(totalDebit || 0);
      } catch (err) {
        console.error("Failed to fetch transactions", err);
      } finally {
        setLoading(false);
      }
    })();
  }, [user, forcedUserId]);

  /* UI */
  if (loading) return <p className="p-6 text-center">Loading transactions…</p>;
  if (txns.length === 0)
    return <p className="p-6 text-center">No transactions found.</p>;

  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold mb-4 text-center">Transaction History</h1>

      <div className="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
        <div className="bg-green-50 p-4 rounded shadow">
          <p className="text-sm text-gray-600">Wallet Balance</p>
          <p className="text-xl font-bold text-green-700">₹ {walletBalance.toFixed(2)}</p>
        </div>
        <div className="bg-blue-50 p-4 rounded shadow">
          <p className="text-sm text-gray-600">Total Credited</p>
          <p className="text-xl font-bold text-blue-700">₹ {totalCredit.toFixed(2)}</p>
        </div>
        <div className="bg-red-50 p-4 rounded shadow">
          <p className="text-sm text-gray-600">Total Debited</p>
          <p className="text-xl font-bold text-red-700">₹ {totalDebit.toFixed(2)}</p>
        </div>
      </div>

      <table className="min-w-full bg-white shadow rounded-lg overflow-hidden text-sm">
        <thead className="bg-gray-100">
          <tr>
            <th className="text-left py-3 px-4 font-semibold text-gray-700">#</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700">Amount</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700">Direction</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700">Counterparty</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700">Status</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700">Date</th>
            <th className="text-left py-3 px-4 font-semibold text-gray-700">Transaction</th>
          </tr>
        </thead>
        <tbody>
          {txns.map((t, idx) => (
            <tr key={t.transactionId} className="border-b last:border-none hover:bg-gray-50">
              <td className="py-3 px-4 text-gray-600">{idx + 1}</td>
              <td className="py-3 px-4 font-semibold text-green-700">
                ₹ {t.amount.toFixed(2)}
              </td>
              <td className="py-3 px-4">
                <span
                  className={`px-2 py-1 text-xs rounded-full font-semibold ${
                    t.direction === "credit"
                      ? "bg-green-100 text-green-700"
                      : "bg-red-100 text-red-700"
                  }`}
                >
                  {t.direction.toUpperCase()}
                </span>
              </td>
              <td className="py-3 px-4 text-gray-600">
                {t.counterparty?.username || "—"}
              </td>
              <td className="py-3 px-4 text-gray-600 capitalize">{t.status}</td>
              <td className="py-3 px-4 text-gray-500">
                {new Date(t.createdAt).toLocaleString()}
              </td>
              <td className="py-3 px-4 text-gray-600 capitalize">{t.transactionId}</td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  );
};

export default TransactionHistory;
