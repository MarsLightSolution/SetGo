...
export default function ChatApp() {
  ...
  const handleAutoConnect = async () => {
    ...
    try {
      ...
      if (userData) {
        ...
        userFromStorage =
          parsedUser.email || parsedUser.username || parsedUser._id;
      }
    } catch (e) {
    ...
    if (!userIdentifier) {
      setConnectionError(
        "No user ID found in localStorage. Please login first."
      );
  ...
  const fetchConversations = async (userIdentifier) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/chat/conversations/${userIdentifier}`
      );
      const data = await response.json();
  ...
  const selectConversation = async (conversation) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/chat/messages/${conversation._id}`
      );
      const data = await response.json();
  ...
  const startConversationFromExternal = async (
    targetUsername,
    productInfo = null
  ) => {
    if (!currentUser) {
      if (isConnecting)
        return setTimeout(
          () => startConversationFromExternal(targetUsername, productInfo),
          1000
        );
  ...
  const getConnectionStatus = () => {
    if (isConnecting)
      return (
        <div className="text-sm text-gray-500 mb-2">Connecting to chat...</div>
      );
    if (connectionError)
      return <div className="text-sm text-red-500 mb-2">{connectionError}</div>;
    if (currentUser)
      return <div className="text-sm text-green-600 mb-2">● Connected</div>;
    return null;
  };
            ...
            {conversations.map((conversation) => {
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    currentUser && selectConversation(conversation)
                  }
              ...
              <div className="bg-white border-t border-gray-200 px-5 py-4 shadow-sm">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    placeholder={
                      currentUser ? "Type a message..." : "Connecting..."
                    }
                    disabled={!currentUser}
